    ...
    const getProficiency = async () => {
      ...
      try {
        const response = await axios.get(
          `https://quiz-app-backend-ehiy.onrender.com/performance/proficiency?uid=${userInfo._id}`,
          config
        );
  ...
  const handleReset = async () => {
    ...
    try {
      const isResetDone = await axios.get(
        `https://quiz-app-backend-ehiy.onrender.com/performance/deletehistory?uid=${userInfo._id}`,
        config
      );
      if (isResetDone) {
        toast({
